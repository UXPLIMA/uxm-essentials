...
1. Download the latest `uxmEssentials-<version>.jar` from the releases page and drop it in your
...
- **Velocity**: drop `uxmEssentials-velocity-<version>.jar` on the proxy and the bus rides a proxy-side broker
  over plugin messaging.
- **Redis**: drop `uxmEssentials-redis-<version>.jar` on each backend and point `network.redis` at a Redis
...
behaviour between minor versions as the surface settles.

The release number is not written here. It lives in the build and on the releases page, and a number
in prose is stale the day after somebody types it: this file said 0.8.2 in eleven places while the
build had moved on. Every jar name above carries `<version>` for the same reason.
