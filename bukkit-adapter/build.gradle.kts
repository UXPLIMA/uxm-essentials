...
dependencies {
    ...
    implementation(libs.uxmlib.item) { isTransitive = false }
    // uxmLib menu engine (dogfood): the spec loader, the renderers, the runtime and the property editors every
    // menu in this plugin is drawn by. This plugin keeps only its own vocabulary on top of it.
    ...
    implementation(libs.uxmlib.nametags) {
        exclude(group = "org.spongepowered")
    }
    ...
    implementation(libs.uxmlib.packet) {
        exclude(group = "org.spongepowered")
    }
    implementation(libs.uxmlib.pipeline) {
        exclude(group = "org.spongepowered")
    }
    // uxmLib update toolkit (dogfood), the opt-in release update-checker. Pulls uxmlib-common only.
    implementation(libs.uxmlib.update) {
        exclude(group = "org.spongepowered")
    }
