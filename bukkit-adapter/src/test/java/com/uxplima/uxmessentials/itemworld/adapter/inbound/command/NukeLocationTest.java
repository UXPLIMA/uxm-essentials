...
/**
 * Regression coverage for itemworld's {@code /nuke} target-location handling (maintainer item 22), mirroring
 * {@link LightningLocationTest}. The storm's centre offset {@code {0,0}} lands a strike exactly on the centre,
 * so a named target's storm is centred on the target (not the caller), and {@code @a} centres a storm on every
 * online player. The strike locations are captured by a recording {@link WorldMock}; the scheduler is a
 * synchronous double so each entity-bound storm is observable inline.
 */
class NukeLocationTest {
    ...
    @Test
    void namedTargetStormCentresOnTheTargetNotTheCaller() {
        PlayerMock caller = addPlayer("Alice", new Location(world, 0, 64, 0));
        PlayerMock target = addPlayer("Bob", new Location(world, 200, 70, -200));

        execute(caller, "nuke Bob");

        assertThat(sink.keys).contains(ItemworldMessageKey.NUKE_DONE);
        assertThat(centres()).contains(blockOf(target.getLocation()));
        assertThat(centres()).doesNotContain(blockOf(caller.getLocation()));
    }

    @Test
    void selectorCentresAStormOnEveryTarget() {
        PlayerMock alice = addPlayer("Alice", new Location(world, 0, 64, 0));
        PlayerMock bob = addPlayer("Bob", new Location(world, 200, 70, -200));

        execute(alice, "nuke @a");

        assertThat(centres()).contains(blockOf(alice.getLocation()), blockOf(bob.getLocation()));
    }

    /** The struck blocks, which include each storm's centre ({@code {0,0}} offset) among its thirteen points. */
    private List<List<Integer>> centres() {
        return world.strikes.stream().map(NukeLocationTest::blockOf).toList();
    }
    ...
    /** Runs scheduled work inline so the entity-bound storm is observable without ticking Folia. */
