...
/**
 * MockBukkit coverage of a workstation command's optional {@code <player>} target through its real Brigadier
 * node. Bare {@code /enderchest} opens for the sender ({@code WORKSTATION_OPENED}); with a target and the
 * {@code uxmessentials.workstation.others} node it opens on the target and confirms with
 * {@code WORKSTATION_OPENED_FOR}; with a target but without the node it is rejected
 * ({@code command.no-permission}) and the target's screen is untouched. The {@link Workstation#ENDERCHEST}
 * station is used because it opens a plain inventory MockBukkit materialises (the {@code MenuType}-backed
 * stations need a titled view the mock server cannot create); the others-target gating it exercises is identical
 * across stations. The scheduler is a synchronous double so the entity-bound open is observable inline.
 */
class WorkstationCommandTest {
    ...
    @Test
    void bareEnderchestOpensForTheSender() {
        sender.setOp(true);

        execute("enderchest", sender);

        assertThat(sink.keys).contains(ItemworldMessageKey.WORKSTATION_OPENED);
        assertThat(sender.getOpenInventory().getType()).isEqualTo(InventoryType.ENDER_CHEST);
        assertThat(target.getOpenInventory().getType()).isNotEqualTo(InventoryType.ENDER_CHEST);
    }

    @Test
    void enderchestOnAnotherPlayerWithTheNodeOpensOnTheTarget() {
        sender.setOp(true); // op satisfies both the station node and the others node

        execute("enderchest Bob", sender);

        assertThat(sink.keys).contains(ItemworldMessageKey.WORKSTATION_OPENED_FOR);
        assertThat(target.getOpenInventory().getType()).isEqualTo(InventoryType.ENDER_CHEST);
        assertThat(sender.getOpenInventory().getType()).isNotEqualTo(InventoryType.ENDER_CHEST);
    }

    @Test
    void enderchestOnAnotherPlayerWithoutTheNodeIsRejected() {
        // Grant only the station node, not uxmessentials.workstation.others.
        sender.addAttachment(MockBukkit.createMockPlugin(), "uxmessentials.workstation.enderchest", true);

        execute("enderchest Bob", sender);

        assertThat(sink.keys).contains(SharedMessageKey.COMMAND_NO_PERMISSION);
        assertThat(sink.keys).doesNotContain(ItemworldMessageKey.WORKSTATION_OPENED_FOR);
        assertThat(target.getOpenInventory().getType()).isNotEqualTo(InventoryType.ENDER_CHEST);
    }

    private void execute(String input, PlayerMock who) {
        CommandDispatcher<CommandSourceStack> dispatcher = new CommandDispatcher<>();
        dispatcher.getRoot().addChild(new WorkstationCommand(Workstation.ENDERCHEST, services()).build());
        try {
    ...
    /** A map-backed {@link ConfigStore} scoped to {@code modules.itemworld}; defaults keep the station enabled. */
    ...
    /** Runs scheduled work inline so the entity-bound open is observable without ticking Folia. */
