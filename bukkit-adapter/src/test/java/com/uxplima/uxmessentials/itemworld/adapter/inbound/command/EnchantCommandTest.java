 ...
 * node is enforced by the Brigadier {@code requires} predicate and is not the subject here.
 */
class EnchantCommandTest {
    ...
    @Test
    void perTypeHolderEnchantsTheItem() {
        permissions.allow("uxmessentials.itemworld.enchant.sharpness");

        execute("enchant sharpness 1");

        assertThat(sink.keys).contains(ItemworldMessageKey.ENCHANT_APPLIED);
        assertThat(player.getInventory().getItemInMainHand().getEnchantmentLevel(Enchantment.SHARPNESS))
                .isEqualTo(1);
    }

    @Test
    void negatedPerTypeNodeIsDeniedAndNothingIsEnchanted() {
        permissions.deny("uxmessentials.itemworld.enchant.sharpness");

        execute("enchant sharpness 1");

        assertThat(sink.keys).contains(SharedMessageKey.COMMAND_NO_PERMISSION);
        assertThat(sink.keys).doesNotContain(ItemworldMessageKey.ENCHANT_APPLIED);
        assertThat(player.getInventory().getItemInMainHand().getEnchantmentLevel(Enchantment.SHARPNESS))
                .isZero();
    }

    @Test
    void aDifferentEnchantmentIsUnaffectedByOneNegatedType() {
        permissions.deny("uxmessentials.itemworld.enchant.sharpness");
        permissions.allow("uxmessentials.itemworld.enchant.unbreaking");

        execute("enchant unbreaking 1");

        assertThat(sink.keys).contains(ItemworldMessageKey.ENCHANT_APPLIED);
        assertThat(player.getInventory().getItemInMainHand().getEnchantmentLevel(Enchantment.UNBREAKING))
                .isEqualTo(1);
    }

    private void execute(String input) {
        CommandDispatcher<CommandSourceStack> dispatcher = new CommandDispatcher<>();
        dispatcher.getRoot().addChild(new EnchantCommand(services()).build());
        try {
