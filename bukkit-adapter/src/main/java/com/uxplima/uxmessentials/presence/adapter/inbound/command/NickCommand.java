...
/**
 * {@code /nick <name>} sets the invoking player's display name, {@code /nick off} clears it back to the account
 * name, and {@code /nick <player> <name>} sets another player's (gated by {@code uxmessentials.nick.others}).
 * The validation, the stamp, and the confirmation are the {@code SetNick}/{@code ClearNick} use cases' job;
 * this handler only maps the arguments and resolves a named target against the online set. The display-name
 * application hops to the target's region thread inside the {@code NickStore}, so the command stays a thin
 * argument mapper.
 */
@NullMarked
