package com.uxplima.uxmessentials.shared.adapter.outbound.teleport;
...
/**
 * The {@link DirectTeleporter} on Paper's {@code teleportAsync}, behind {@code /hologram teleport} and
 * {@code /npc teleport}. An operator inspecting a marker is moved straight to it: an admin convenience, not a
 * gated player teleport, so it deliberately bypasses the teleport context's cooldown and warmup and hops
 * directly.
 *
 ...
 */
@NullMarked
public final class BukkitDirectTeleporter implements DirectTeleporter {

    private final Scheduler scheduler;
    private final Logger log;

    public BukkitDirectTeleporter(Scheduler scheduler, Logger log) {
    ...
    private void hop(PlayerRef who, Position destination) {
        ...
        if (world == null) {
            log.warn(
                    "teleport destination world {} is not loaded for {}",
