 ...
 * the SDK classes. <b>Reflective</b> providers — uxmClaims, GriefDefender, ExcellentClaims, XClaim and
 * Homestead — have no compile dependency at all and reach their API purely by reflection: GriefDefender
 * publishes only a GC-prone commit-pinned JitPack build, ExcellentClaims is premium with no public
 * coordinate, XClaim's JitPack build fails, and Homestead ships only to authenticated GitHub Packages — so
 * a pinned canon-verifiable {@code compileOnly} dependency is not available for any of them, while each
