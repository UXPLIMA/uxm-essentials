 ...
 * Implemented by {@link BusClient}; a context never holds the client directly, only this narrow publish
 * surface. The publisher stamps the origin and routes the frame — a disabled bus makes every publish a
 * no-op, so a broadcasting decorator is safe to wire unconditionally and the single-server path is unchanged.
 *
