 ...
 * /sit} command plus the three listeners the plugin registers. The seat port is a real, tagged, non-persistent
 * seat entity ({@link BukkitSeatPort}); the region gate is the permissive Phase-1 {@link AllowAllRegionGate} (the
 * Phase-5 claim/WorldGuard gate slots in here with no reach into the use cases). The context persists nothing — a
 * pose is transient state in {@link PoseSessions} — so there is no repository or migration.
 *
...
public final class PosesWiring {
    ...
    public static Wired wire(Plugin plugin, ModuleContext ctx) {
        ...
        List<CommandRegistration> commands = List.of(new SitCommand(
                startSit, sittableBlocks, kernel.messages(), config.sitOnBlocks(), config.maxDistance()));
        ...
        return new Wired(commands, listeners, seats, sessions);
    }
     ...
     */
    public record Wired(
            List<CommandRegistration> commands, List<Listener> listeners, BukkitSeatPort seats, PoseSessions sessions) {
