...
/**
 * The {@link NpcTeleporter} on Paper's {@code teleportAsync}, for {@code /npc teleport}. An operator inspecting an
 * NPC is moved straight there — this is an admin convenience, not a gated player teleport, so it deliberately
 * bypasses the teleport context's cooldown/warmup and hops directly.
 *
 ...
 */
@NullMarked
public final class NpcTeleportAdapter implements NpcTeleporter {

    private final Scheduler scheduler;
    private final Logger log;

    public NpcTeleportAdapter(Scheduler scheduler, Logger log) {
    ...
    private void hop(PlayerRef who, Position destination) {
        ...
        if (world == null) {
            log.warn(
                    "npc teleport world {} is not loaded for {}",
