...
@NullMarked
public final class BankChatPromptListener implements Listener {
    ...
    public BankChatPromptListener(Messages messages) {
        this.promptCancel = new PromptCancel(Objects.requireNonNull(messages, "messages"));
    }
    ...
    public void onChat(AsyncChatEvent event) {
        ...
        prompts.take(player.getUniqueId()).ifPresent(callback -> {
            ...
            if (promptCancel.isCancel(viewer, text)) {
                promptCancel.sendCancelled(player, viewer);
                return;
            }
