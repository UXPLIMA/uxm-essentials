 ...
 * editors do not cover. The move-here button is the motivating case: it reads the operator's current position (a
 * live {@link Player} read that must happen on the region thread) and re-anchors the warp there, which is not a
 * value the viewer types or cycles.
 *
 ...
 * redrawing. It carries no domain logic itself — the handler is a thin call into the module's existing write
 * path. The value lore is a fixed catalog hint (the button has no editable "current value").
 */
@NullMarked
final class PlayerWarpActionButton implements EditableProperty {
