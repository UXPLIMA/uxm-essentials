 ...
 * {@link HologramEditorView} on click. A create button prompts for a new name and creates a fresh text
...
public final class HologramListView {
    ...
    public HologramListView(
        ...
        this.view = EntityListView.<Hologram>builder()
                ...
                .onCreate(HologramsMessageKey.HOLOGRAM_GUI_LIST_CREATE, player -> promptCreate(player, services))
                .build();
    }
    ...
    private void promptCreate(Player player, com.uxplima.uxmessentials.holograms.adapter.HologramServices services) {
        ...
        textInput.prompt(
                ...
                text -> handleCreate(player, viewer, services, text),
                () -> view.open(player, viewer));
    }
