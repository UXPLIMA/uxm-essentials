 ...
 * nest that category under another: every category except the one being edited is listed, since a category can never
 * be its own parent, plus a clear button and a back button to the settings panel.
 *
 * <p>The candidates are the {@code kits:category-parent-selector} list source, snapshotted at open and handed to the
 * engine as the menu subject. The candidate's icon, name and lore reach the spec through the {@code kit_parent_*}
 * placeholders because they are authored per category; the geometry and the fixed buttons live in
 * {@code modules/kits/gui/kit-category-parent-selector.conf}.
 */
@NullMarked
public final class KitCategoryParentSelectorMenu {

    /** The engine spec id this menu registers and opens under. */
    public static final String SPEC_ID = "kit-category-parent-selector";

    private static final String SPEC_RESOURCE = "modules/kits/gui/kit-category-parent-selector.conf";
    ...
    public KitCategoryParentSelectorMenu(
            Menus menus,
            Messages messages,
            Scheduler scheduler,
    ...
    public void register(MenuBindings bindings, Path dataFolder, Logger log) {
        ...
        bindings.placeholder(
                "kit_parent_icon",
                ctx -> KitCategoryIcons.material(candidateOf(ctx)).name());
        bindings.placeholder("kit_parent_name", ctx -> candidateOf(ctx).displayName());
        bindings.placeholder("kit_parent_lore", this::lore);
    ...
    /**
     * The bound candidate's lore as one string the renderer splits per line: its own lore lines, a spacer, its id
     * line, a spacer, then the click hint. This is what the bespoke grid drew, line for line.
     */
    private String lore(MenuContext ctx) {
        KitCategory candidate = candidateOf(ctx);
    ...
    private static KitCategory editedOf(MenuActionContext ctx) {
     ...
     */
    public record Selection(KitCategory edited, List<KitCategory> candidates) {
