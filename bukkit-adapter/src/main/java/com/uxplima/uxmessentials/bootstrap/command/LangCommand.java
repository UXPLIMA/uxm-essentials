...
public final class LangCommand implements CommandRegistration {
    ...
    private @org.jspecify.annotations.Nullable Player playerOf(CommandContext<CommandSourceStack> ctx) {
        ...
        PlayerRef consoleRef = new PlayerRef(new java.util.UUID(0L, 0L), sender.getName());
        sender.sendMessage(net.kyori.adventure.text.minimessage.MiniMessage.miniMessage()
                .deserialize(messages.resolve(consoleRef, SharedMessageKey.COMMAND_PLAYERS_ONLY, Map.of())));
        return null;
    }
}
