 ...
 * {@link VaultSize} in rows, and the {@code lastTouched} instant — plus the opaque {@link VaultContents} the
 * adapter (de)serialises. The aggregate never sees a Bukkit {@code ItemStack}; it holds the already-serialized
 * payload, which is the only part the architecture invariant lets serialize (docs/01-architecture.md).
 *
...
public final class Vault {
    ...
    private Vault(VaultId id, VaultSize size, VaultContents contents, Instant lastTouched) {
        this.id = id;
        this.size = size;
    ...
    /** Rebuild a vault from its persisted row. */
    public static Vault of(VaultId id, VaultSize size, VaultContents contents, Instant lastTouched) {
        return new Vault(
     ...
     * until the player stores something and closes it.
     */
    public static Vault allocate(VaultId id, VaultSize size, Instant at) {
        return of(id, size, VaultContents.empty(), Objects.requireNonNull(at, "at"));
    }
    ...
    public Vault openedAt(VaultSize newSize) {
        ...
        return newSize.equals(size) ? this : new Vault(id, newSize, contents, lastTouched);
    }
    ...
    public Change withContents(PlayerRef owner, VaultContents newContents, Instant at) {
        ...
        Vault saved = new Vault(id, size, newContents, at);
