...
/**
 * Outbound port over the client-visible rendering of a free pose — the lie-down body pose behind {@code /lay} and
 * {@code /bellyflop}, and the yaw rotation behind {@code /spin}. It sits alongside {@link SeatPort}: the seat entity
 * holds the player still, this port makes them <em>look</em> posed. The seat and the render are separate concerns so
 * the one-session invariant, the anchor, and the cleanup all stay in one place while the visual is swapped
 * independently.
 *
...
public interface PosePort {
    ...
    /**
     * Render {@code who} in {@code pose} — a {@link PoseType#LAY}/{@link PoseType#BELLYFLOP} lie-down or a
     * {@link PoseType#SPIN} rotation. Only the free poses are valid here; the seat/player sits are rendered by the
     * client already and never reach this port. Called after the player is mounted on their anchor seat.
     */
