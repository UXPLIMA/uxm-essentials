...
/**
 * Pairs the {@link Messages} resolution port with the {@link MessageSink} delivery port so a staff use case
 * sends a {@link MessageKey} to a viewer in one call — the context's plain feedback channel (the staff-mode
 * on/off toggle confirmation, the already-in-mode line). Resolution happens in the viewer's locale and
 * delivery hops to the viewer's region thread, silently no-opping when they are offline. Mirrors the
 * messaging context's {@code MessagingNotifier} rather than sharing one.
 */
public final class StaffNotifier {
    ...
    public StaffNotifier(Messages messages, MessageSink sink) {
