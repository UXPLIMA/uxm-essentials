...
/**
 * Which kind of click fires an {@link NpcAction}. A player's interaction with an NPC arrives as either an attack
 * (the left mouse button) or an interact (the right button), and an action declares which it responds to:
...
public enum ClickTrigger {
    ...
    /** Fires only when the player attacks (left-clicks) the NPC. */
    LEFT_CLICK,

    /** Fires only when the player interacts with (right-clicks) the NPC. */
    RIGHT_CLICK,
