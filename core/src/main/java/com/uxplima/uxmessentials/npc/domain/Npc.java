 ...
 * stored as raw text here — running it on interaction is an adapter concern wired in a later sub-phase, so the
 * domain only carries the binding.
 *
 ...
 */
public record Npc(
        NpcName name, Position location, @Nullable NpcSkin skin, @Nullable String clickCommand, Instant createdAt) {
    ...
    /** A new NPC created now at {@code location} with the given (possibly {@code null}) skin and no command. */
    public static Npc create(NpcName name, Position location, @Nullable NpcSkin skin, Instant createdAt) {
        return new Npc(name, location, skin, null, createdAt);
    }
    ...
    public Npc movedTo(Position newLocation) {
        ...
        return new Npc(name, newLocation, skin, clickCommand, createdAt);
    }
    ...
    public Npc withSkin(@Nullable NpcSkin newSkin) {
        return new Npc(name, location, newSkin, clickCommand, createdAt);
    }
    ...
    public Npc withClickCommand(@Nullable String newCommand) {
        return new Npc(name, location, skin, newCommand, createdAt);
    }
