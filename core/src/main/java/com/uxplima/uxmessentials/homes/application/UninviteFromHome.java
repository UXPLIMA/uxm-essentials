...
/**
 * {@code /home uninvite <slot> <player>}: revoke a previously granted invite to one of the owner's homes. The
 * call is idempotent — revoking an invite that was never granted is a no-op that still reports success — so
 * the owner gets the same feedback whether or not the player was on the guest list.
 */
