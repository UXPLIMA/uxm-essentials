 ...
 * {@link WorldSpec}, the mutable management facets A owns (alias, auto-load, the adopted flag, and
 * the Bukkit uid once known), and the per-world {@link WorldSettings} bag (properties, gamerules,
 * spawn) owned by sub-project B. Immutable; mutations return a new instance.
 */
public record ManagedWorld(
    ...
    public ManagedWorld withAlias(Optional<String> newAlias) {
        return new ManagedWorld(name, spec, newAlias, autoLoad, adopted, knownUid, createdAt, createdBy, settings);
    }

    public ManagedWorld withAutoLoad(boolean newAutoLoad) {
        return new ManagedWorld(name, spec, alias, newAutoLoad, adopted, knownUid, createdAt, createdBy, settings);
    }
    ...
    public ManagedWorld withSettings(WorldSettings newSettings) {
        return new ManagedWorld(
                name,
                spec,
                alias,
                ...
                createdAt,
                createdBy,
                Objects.requireNonNull(newSettings, "settings"));
    }
}
