...
/**
 * A thin application helper that pairs the {@link Messages} resolution port with the {@link MessageSink}
 * delivery port, so a use case sends a {@link MessageKey} to a viewer in one call instead of resolving
 * then delivering at every site. Resolution happens in the viewer's locale; delivery hops to the
 * viewer's region thread and silently no-ops when they are offline.
 */
public final class PlayerNotifier {
    ...
    public PlayerNotifier(Messages messages, MessageSink sink) {
