 ...
 * movehere}. The hologram is resolved here and its location handed to the {@link HologramTeleporter} port, which
 * performs the region-aware async hop. A name no hologram exists at is rejected with
 * {@link HologramError#NOT_FOUND}. The operator-only permission is enforced at the adapter gate.
 */
public final class TeleportToHologram {

    private final HologramRepository repository;
    private final HologramTeleporter teleporter;
    private final HologramNotifier notifier;

    public TeleportToHologram(HologramRepository repository, HologramTeleporter teleporter, HologramNotifier notifier) {
