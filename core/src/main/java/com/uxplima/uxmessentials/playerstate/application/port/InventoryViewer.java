 ...
 * the {@link PlayerEffects} verbs, which act on the subject, these open a live container <em>for the viewer</em>
 * that shows the subject's contents, so the open must run on the viewer's owning region/entity thread (that is
 * where their open inventory lives). Both players must be online; an offline viewer or subject is a silent
 * no-op.
 */
