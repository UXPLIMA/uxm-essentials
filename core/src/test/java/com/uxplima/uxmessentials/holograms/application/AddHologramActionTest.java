...
class AddHologramActionTest {
    ...
    void setUp() {
        ...
        add = new AddHologramAction(repository, new HologramNotifier(new HologramTestSupport.KeyMessages(), sink));
        actor = new PlayerRef(UUID.randomUUID(), "Operator");
    }

    @Test
    void appendsTheActionSavesAndNotifies() {
        repository.save(hologram("spawn"));

        Result<Unit, HologramError> result = add.add(
                actor,
                HologramName.of("spawn"),
                new ClickAction(ClickTrigger.RIGHT_CLICK, ClickActionType.MESSAGE, "hi"));

        assertThat(result.isOk()).isTrue();
        Hologram updated = repository.find(HologramName.of("spawn")).orElseThrow();
        assertThat(updated.actions()).hasSize(1);
        assertThat(updated.actions().get(0).value()).isEqualTo("hi");
        assertThat(sink.textFor(actor)).contains(HologramsMessageKey.HOLOGRAM_ACTION_ADDED.key());
    }
