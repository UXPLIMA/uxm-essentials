...
class InsertHologramActionTest {

    private static final WorldRef WORLD = new WorldRef(UUID.randomUUID(), "world");
    private static final ClickAction NEW = new ClickAction(ClickTrigger.ANY, ClickActionType.MESSAGE, "inserted");

    private FakeHologramRepository repository;
    private CapturingSink sink;
    private InsertHologramAction insert;
    ...
    @Test
    void insertsBeforeTheIndexAndNotifies() {
        repository.save(withTwoActions("spawn"));

        Result<Unit, HologramError> result = insert.insert(actor, HologramName.of("spawn"), 1, false, NEW);

        assertThat(result.isOk()).isTrue();
        Hologram updated = repository.find(HologramName.of("spawn")).orElseThrow();
        assertThat(updated.actions()).hasSize(3);
        assertThat(updated.actions().get(0).value()).isEqualTo("inserted");
        assertThat(sink.textFor(actor)).contains(HologramsMessageKey.HOLOGRAM_ACTION_INSERTED.key());
    }

    @Test
    void insertsAfterTheIndex() {
        repository.save(withTwoActions("spawn"));

        insert.insert(actor, HologramName.of("spawn"), 1, true, NEW);

        Hologram updated = repository.find(HologramName.of("spawn")).orElseThrow();
        assertThat(updated.actions().get(1).value()).isEqualTo("inserted");
    }
    ...
    void rejectsAnOutOfRangeIndex() {
        ...
        Result<Unit, HologramError> result = insert.insert(actor, HologramName.of("spawn"), 9, false, NEW);
    ...
    void rejectsAnUnknownName() {
        Result<Unit, HologramError> result = insert.insert(actor, HologramName.of("ghost"), 1, false, NEW);
