...
class MoveHologramActionTest {
    ...
    void setUp() {
        ...
        move = new MoveHologramAction(repository, new HologramNotifier(new HologramTestSupport.KeyMessages(), sink));
        actor = new PlayerRef(UUID.randomUUID(), "Operator");
    }

    @Test
    void movesTheActionUpAndNotifies() {
        repository.save(withTwoActions("spawn"));

        Result<Unit, HologramError> result = move.move(actor, HologramName.of("spawn"), 2, true);
        ...
        assertThat(updated.actions().get(1).value()).isEqualTo("first");
        assertThat(sink.textFor(actor)).contains(HologramsMessageKey.HOLOGRAM_ACTION_MOVED.key());
    }
    ...
    void rejectsAnOutOfRangeIndex() {
        ...
        Result<Unit, HologramError> result = move.move(actor, HologramName.of("spawn"), 9, true);
    ...
    void rejectsAnUnknownName() {
        Result<Unit, HologramError> result = move.move(actor, HologramName.of("ghost"), 2, true);
